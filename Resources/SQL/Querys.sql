...
SELECT T3.ID_COMPRAS,T.FK_USER AS USR,T2.NOMBRE,T3.CANTIDAD,T2.PRECIO,T3.TOTAL,T2.IMG FROM Vendedor AS T
...
WHERE T4.FK_USER ='Dianita' ORDER BY T3.ID_COMPRAS;

--Listar Ventas de Usuario
SELECT T3.ID_COMPRAS,T4.FK_USER AS USR,T2.NOMBRE,T3.CANTIDAD,T2.PRECIO,T3.TOTAL,T2.IMG FROM Vendedor AS T
INNER JOIN Producto AS T2 
ON T.ID_VENDEDOR = T2.FK_VENDEDOR
INNER JOIN Compras AS T3
ON T3.FK_PRODUCTO =T2.ID_PRODUCTO
INNER JOIN Comprador AS T4
ON T3.FK_COMPRADOR=T4.ID_COMPRADOR
WHERE T.FK_USER ='Jessy' ORDER BY T3.ID_COMPRAS;
